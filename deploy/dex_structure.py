...
class DeployStructureArguments:
    ...
    def add_clean_contract_deploy_arguments(cls, parser: ArgumentParser):
        parser.add_argument("--rewa-wraps", action="store_true", help="Deploy clean REWA wraps")
...
class DeployStructure:
    def __init__(self):
        ...
        self.dcdt_contract = DCDTContract(config.TOKENS_CONTRACT_ADDRESS)

        self.contracts: Dict[str, ContractStructure] = {
            config.REWA_WRAPS:
                ContractStructure(config.REWA_WRAPS, RewaWrapContract, config.REWA_WRAP_BYTECODE_PATH,
                                  self.rewa_wrap_deploy, False),
    ...
    # CONTRACT DEPLOYERS ------------------------------
    def rewa_wrap_deploy(self, contracts_index: str, deployer_account: Account, network_providers: NetworkProviders):
        ...
        for contract_config in contract_structure.deploy_structure_list[num_already_deployed:]:
            ...
            # deploy contract
            deployed_contract = RewaWrapContract(wrapped_token)
            ...
            if not network_providers.check_deploy_tx_status(tx_hash, contract_address, "rewa wrapper"): return
            deployed_contract.address = contract_address
            log_step_pass(f"REWA wrap contract address: {contract_address}")

            # Set special role on wrapped tokens
            tx_hash = self.dcdt_contract.set_special_role_token(deployer_account,
                                                                network_providers.proxy,
                                                                [wrapped_token, contract_address,
                                                                 "DCDTRoleLocalMint", "DCDTRoleLocalBurn"])
    ...
    def proxy_deploy(self, contracts_index: str, deployer_account: Account, network_providers: NetworkProviders):
        ...
        for config_proxy in contract_structure.deploy_structure_list[num_already_deployed:]:
            ...
            self.dcdt_contract.set_special_role_token(deployer_account, network_providers.proxy,
                                                      [asset, deployed_proxy_contract.address, "DCDTRoleLocalMint"])
            if not network_providers.check_complex_tx_status(tx_hash, "set special role on unlocked asset"): return

            self.dcdt_contract.set_special_role_token(deployer_account, network_providers.proxy,
                                                      [asset, deployed_proxy_contract.address, "DCDTRoleLocalBurn"])
            if not network_providers.check_complex_tx_status(tx_hash, "set special role on unlocked asset"): return
    ...
    def farm_deploy(self, contracts_index: str, deployer_account: Account, network_providers: NetworkProviders):
        ...
        for config_farm in contract_structure.deploy_structure_list[num_already_deployed:]:
            ...
            if locked_asset_contract is not None:
                ...
                tx_hash = self.dcdt_contract.set_special_role_token(deployer_account, network_providers.proxy,
    ...
    def price_discovery_deploy(self, contracts_index: str, deployer_account: Account, network_providers: NetworkProviders):
        ...
        for config_pd in contract_structure.deploy_structure_list[num_already_deployed:]:
            ...
            if hex_to_string(redeem_token_hex) == "REWA":
    ...
    def position_creator_deploy(self, contracts_index: str, deployer_account: Account, network_providers: NetworkProviders):
        ...
        num_already_deployed = len(self.contracts[contracts_index].deployed_contracts)
        for contract_config in contract_structure.deploy_structure_list[num_already_deployed:]:
            if contract_config.get('rewa_wrap_address'):
                rewa_wrapper_contract_address = contract_config.get('rewa_wrap_address')
            else: 
                rewa_wrapper_contract_address = self.contracts[config.REWA_WRAPS].get_deployed_contract_by_index(contract_config['rewa_wrap']).address
            ...
            position_creator_contract = PositionCreatorContract(
                "", rewa_wrapper_contract_address, router_contract_address
            )
    ...
    def locked_token_position_creator_deploy(self, contracts_index: str, deployer_account: Account, network_providers: NetworkProviders):
        ...
        for contract_config in contract_structure.deploy_structure_list[num_already_deployed:]:
            ...
            rewa_wrapper_contract_address = self.contracts[config.REWA_WRAPS].get_deployed_contract_by_index(contract_config['rewa_wrap']).address
            moa_wrewa_pair_address = self.contracts[config.PAIRS_V2].get_deployed_contract_by_index(contract_config['moa_wrewa_pair']).address
            moa_wrewa_farm_address = self.contracts[config.FARMS_V2].get_deployed_contract_by_index(contract_config['moa_wrewa_farm']).address
            ...
            locked_token_position_creator_contract = LockedTokenPositionCreatorContract(
                "", energy_factory_address, rewa_wrapper_contract_address, 
                moa_wrewa_pair_address, moa_wrewa_farm_address, proxy_dex_address,
                router_contract_address
            )
            tx_hash, contract_address = locked_token_position_creator_contract.contract_deploy(
                ...
                [
                    ...
                    rewa_wrapper_contract_address,
                    moa_wrewa_pair_address,
                    moa_wrewa_farm_address,
                    proxy_dex_address,
                    router_contract_address
                ]
            )
    ...
    def composable_tasks_deploy(self, contracts_index: str, deployer_account: Account, network_providers: NetworkProviders):
        ...
        for contract_config in contract_structure.deploy_structure_list[num_already_deployed:]:
            ...
            # setup contract
            tx_hash = contract.set_wrap_rewa_address(deployer_account, network_providers.proxy, [rewa_wrapper_contract_address])
            if not network_providers.check_simple_tx_status(tx_hash, "set wrap rewa address"):
                return
