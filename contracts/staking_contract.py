...
from utils.utils_tx import NetworkProviders, DCDTToken, multi_dcdt_endpoint_call, deploy, upgrade_call, endpoint_call
...
class StakingContract(BaseFarmContract, BaseBoostedContract, BaseSCWhitelistContract):
    ...
    def stake_farm(self, network_provider: NetworkProviders, user: Account, event: EnterFarmEvent,
        ...
        return multi_dcdt_endpoint_call(stake_farm_fn, network_provider.proxy, gas_limit, user,
    ...
    def unstake_farm(self, network_provider: NetworkProviders, user: Account, event: ExitFarmEvent) -> str:
        ...
        tokens = [DCDTToken(event.farm_token, event.nonce, event.amount)]
        args = [tokens]
        if self.version == StakingContractVersion.V3Boosted:
            args.append(event.exit_amount)

        return multi_dcdt_endpoint_call(unstake_fn, network_provider.proxy, gas_limit, user,
    ...
    def unbond_farm(self, network_provider: NetworkProviders, user: Account, event: ExitFarmEvent) -> str:
        ...
        return multi_dcdt_endpoint_call(unbond_fn, network_provider.proxy, gas_limit, user,
    ...
    def claim_rewards(self, network_provider: NetworkProviders, user: Account, event: ClaimRewardsFarmEvent) -> str:
        ...
        tokens = [DCDTToken(self.farm_token, event.nonce, event.amount)]
        sc_args = [tokens]
        return multi_dcdt_endpoint_call(claim_fn, network_provider.proxy, gas_limit, user,
    ...
    def compound_rewards(self, network_provider: NetworkProviders, user: Account, event: CompoundRewardsFarmEvent) -> str:
        ...
        tokens = [DCDTToken(self.farm_token, event.nonce, event.amount)]
        sc_args = [tokens]
        return multi_dcdt_endpoint_call(compound_fn, network_provider.proxy, gas_limit, user,
    ...
    def topup_rewards(self, deployer: Account, proxy: ProxyNetworkProvider, rewards_amount: int):
        ...
        tokens = [DCDTToken(self.farmed_token, 0, rewards_amount)]
        sc_args = [tokens]
        return multi_dcdt_endpoint_call(function_purpose, proxy, gas_limit, deployer,
