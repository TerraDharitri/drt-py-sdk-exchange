...
from utils.utils_tx import prepare_contract_call_tx, send_contract_call_tx, NetworkProviders, DCDTToken, \
    multi_dcdt_endpoint_call, deploy, endpoint_call
...
class PriceDiscoveryContract(DEXContractInterface):
    ...
    def deposit_liquidity(self, network_provider: NetworkProviders, user: Account, event: DepositPDLiquidityEvent) -> str:
        ...
        gas_limit = 10000000
        tokens = [DCDTToken(event.deposit_token, 0, event.amount)]
        sc_args = [tokens]
    ...
    def withdraw_liquidity(self, network_provider: NetworkProviders, user: Account, event: WithdrawPDLiquidityEvent) -> str:
        ...
        gas_limit = 10000000
        tokens = [DCDTToken(event.deposit_lp_token, event.nonce, event.amount)]
        sc_args = [tokens]
        return multi_dcdt_endpoint_call(function_purpose, network_provider.proxy, gas_limit, user,
