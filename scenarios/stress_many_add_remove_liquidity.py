...
from dharitri_py_sdk import Address, Transaction, ProxyNetworkProvider

from ported_arrows.stress.contracts.transaction_builder import \
    transfer_multi_dcdt_and_execute
...
def create_add_liquidity(pair: Address, caller: Account, token_one: str, token_two: str, network: NetworkConfig) -> Transaction:
    ...
    args = [1, 1]
    gas_limit = 8400000
    transaction = transfer_multi_dcdt_and_execute(pair, caller, transfers, "addLiquidity", args, gas_limit, network)
    return transaction
...
def create_remove_liquidity(pair: Address, caller: Account, token_lp: str, network: NetworkConfig) -> Transaction:
    ...
    gas_limit = 750000
    transaction = transfer_multi_dcdt_and_execute(pair, caller, transfers, "remove_liquidity", args, gas_limit, network)
    return transaction
