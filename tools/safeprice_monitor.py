...
def main(cli_args: List[str]):
    ...
    contract_data_fetcher = PairContractDataFetcher(Address(pair_contract.address, "drt"), proxy.url)
    if args.view_contract:
        view_data_fetcher = PairContractDataFetcher(Address(args.view_contract, "drt"), proxy.url)

    dcdt_token_payment_schema = {
        'token_identifier': 'string',
        'token_nonce': 'u64',
        'amount': 'biguint',
    }
    zero_dcdt_token_result = {
        'token_identifier': 'string',
        'token_nonce': 0,
        'amount': 0,
    }
    ...
    while i:
        ...
        decoded_attrs: dict = zero_dcdt_token_result
        if hex_val:
            decoded_attrs = decode_merged_attributes(hex_val, dcdt_token_payment_schema)
        ...
        if args.view_contract:
            hex_val = view_data_fetcher.get_data("getSafePriceByRoundOffset",
                                                 [Address(pair_contract.address, "drt").pubkey, 100,
                                                  bytes.fromhex(view_payload)])
            ten_min_avg: dict = zero_dcdt_token_result
            if hex_val:
                ten_min_avg = decode_merged_attributes(hex_val, dcdt_token_payment_schema)

            hex_val = view_data_fetcher.get_data("getSafePriceByTimestampOffset",
                                                 [Address(pair_contract.address, "drt").pubkey, 600,
                                                  bytes.fromhex(view_payload)])
            ten_min_avg_2: dict = zero_dcdt_token_result
            if hex_val:
                ten_min_avg_2 = decode_merged_attributes(hex_val, dcdt_token_payment_schema)

            hex_val = view_data_fetcher.get_data("getSafePriceByTimestampOffset",
                                                 [Address(pair_contract.address, "drt").pubkey, 1200,
                                                  bytes.fromhex(view_payload)])
            twenty_min_avg: dict = zero_dcdt_token_result
            if hex_val:
                twenty_min_avg = decode_merged_attributes(hex_val, dcdt_token_payment_schema)
