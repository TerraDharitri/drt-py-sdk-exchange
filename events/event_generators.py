...
from utils.utils_chain import (prevent_spam_crash_numbat_proxy_go,
...
def generateRandoMoaitFarmEvent(context: Context):
    userAccount = context.get_random_user_account()
    farmContract = context.get_random_farm_contract()
...
def generateAddLiquidityProxyEvent(context: Context):
    ...
    tokenA = context.wrapped_rewa_tkid
    tokenB = context.lkmoa_tkid
    ...
    try:
        ...
        for token in tokens['dcdts'].keys():
            if tokenB in token:
                nonce = int(tokens['dcdts'][token]['nonce'])
                break
